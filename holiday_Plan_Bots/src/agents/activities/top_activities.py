...
prompt = PromptTemplate(
    template="""
        ...
        The question that SerpAPI has to answer: What are the top 3 tourist activities in {city} on {date}?\n
...
async def get_top_activity(ctx: Context, sender: str, msg: TopActivities):
    ...
    try:
        ...
        # Log the type of the result object to diagnose the warning
        ctx.logger.info(f"Type of result object: {type(result)}")
